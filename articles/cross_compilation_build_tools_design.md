...
- The command downloads a base ROS 2 Docker image for the target platform, and
  ...
  missing. Note we can run `rosdep` in Docker for platform like ARM-HF thanks
...
could use `cc-build` for easy cross-compilation minimizing the usage
...
`cc-test` command could further simplify using that image.
